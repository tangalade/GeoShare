...
int main( int argc, char *argv[])
{
  ...
  switch (req_type)
  {
	...
	break;
      ...
      for ( int i=0; i<num_shares; i++ )
	threads_g.at(i) = std::thread(put_concurrent, buckets[i], object_name, (char *)frags[i].c_str(), frags[0].length());
      for ( int i=0; i<num_shares; i++ )
	threads_g[i].join();
...
//      }
	...
	for ( int i=0; i<num_shares; i++ )
	{
	  threads_g.at(i) = std::thread(get_concurrent, buckets[i], object_name, frags[i], &object_size);
	  threads_g[i].detach();
	}
