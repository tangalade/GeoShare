...
)
{
  ...
  frag.clear();
  //  std::cerr << full_name << " started" << std::endl;
  if ( (buf = get_object((char *)full_name.c_str(), (int *)&frag_len)) )
...
//  }
//  else
//    std::cerr << "down bucket attempted" << std::endl;

//  std::cerr << full_name << " finished" << std::endl;
...
int main( int argc, char *argv[])
{
  ...
  { // process command line params
    ...
    {
      std::cerr << "Invalid destinations" << std::endl;
      return 1;
    }
...
//      }
	...
	{
	  // EDIT
	  threads_g.at(i) = std::thread(get_concurrent, buckets[i%buckets.size()], object_name, std::ref(frags[i]), down, &object_size);
	  threads_g[i].detach();
	}
	...
	{
	  cv_g.wait(lck);
	  //	  std::cerr << finished_g << " of " << threshold << std::endl;
	}
#else
