...
struct Encoding {
  ...
  Encoding() : threshold(1), num_shares(1), num_replicas(0), type(ENCODING_TYPE_MAX){};
  const std::string to_string() const
  {
    ...
    case ENCODING_TYPE_AES_SSS:
      name << "(" << threshold << "," << num_shares << "," << num_replicas << ")";
      break;
    }
    return name.str();
  };
  bool parse( std::string arg )
  {
    ...
    if ( params.front() != '(' || params.back() != ')' || params.length() <= 2 )
      return false;
    ...
    {
    case ENCODING_TYPE_SPLIT:
      if ( !(num_shares = atoi(params.c_str())) )
	return false;
      else
      {
    ...
    case ENCODING_TYPE_XOR:
      ...
      if ( !(comma_pos < params.length()-1 && comma_pos > 0) )
	return false;
      std::string first = params.substr(0,comma_pos);
      std::string second = params.substr(comma_pos+1);
	...
	return false;
      ...
      comma_pos = second.find(",");
      if ( !(comma_pos < second.length()-1 && comma_pos > 0) )
	return false;
      first = second.substr(0,comma_pos);
      second = second.substr(comma_pos+1);
      if ( !(num_shares = atoi(first.c_str())) )
	return false;
      if ( !(num_replicas = atoi(second.c_str())) )
	return false;
      break;
      }
    default:
      std::cerr << "Something weird happened during encoding parse" << std::endl;
      return false;
    }
...
bool encode_object( std::string object, Encoding enc, std::vector<std::string>& frags )
{
  switch (enc.type)
  {
    ...
    case ENCODING_TYPE_AES_SSS:
    {
      ...
      ciphertext = AESEncrypt(object, aes_key);
      for ( int i=0; i<enc.num_replicas; i++ )
	frags.push_back(ciphertext);
...
int main( int argc, char *argv[] )
{
  ...
  if ( options[ARGS_SOURCE] )
    source = options[ARGS_SOURCE].arg;
  {
...
//    if ( buckets.size() < enc_type_.num_shares+enc_type_.num_replicas )
//    {
//      std::cerr << "Invalid destinations: "
//		<< "need " << enc_type_.num_shares + enc_type_.num_replicas
...
//      buckets.insert(buckets.begin(),prefix + "apne1");
  ...
  if ( enc_type_.type == ENCODING_TYPE_AES_SSS )
    for ( int i=0; i<enc_type_.num_replicas; i++ )
